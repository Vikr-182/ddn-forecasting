...
from ddn.ddn.ddn.pytorch.node import AbstractDeclarativeNode
...
class TrajNet(nn.Module):
    def init(self, opt_layer, P, Pdot, input_size=2, hidden_size=16, embedding_size = 128, output_size=2, nvar=11, t_obs=8, num_layers = 1):
...
train_dataset = ArgoverseDataset("/scratch/forecasting2/forecasting_features_train.pkl", t_obs=20, dt=0.3)
train_loader = DataLoader(train_dataset, batch_size=20, shuffle=True, num_workers=0)

test_dataset = ArgoverseDataset("/scratch/forecasting2/forecasting_features_val.pkl", t_obs=20, dt=0.3)
