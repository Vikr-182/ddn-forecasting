...
# %%
"""
This is the latest optimizer node that doesn't take psidot_fin into account. 
- Num here is set to 16, and this should be changed depending on the dataset being used. 
- The weight smoothness parameter has been tweaked from the previous value of 5.0.
- If the initial velocity constraint cost is required, please uncomment the 111th and 149th lines.
"""
class OPTNode(AbstractDeclarativeNode):
    def __init__(self, rho_eq=1.0, rho_goal=1.0, rho_nonhol=1.0, rho_psi=1.0, maxiter=500, weight_smoothness=1.0, weight_smoothness_psi=1.0, t_fin=2.0, num=16):
        super().__init__()
        self.rho_eq = rho_eq
...
#     def init(self, opt_layer, P, Pdot, input_size=32, hidden_size=64, output_size=3, nvar=11, t_obs=8):
#         super(TrajNet, self).init()
#         self.nvar = nvar
#         self.t_obs = t_obs
...
#         psi_pred = torch.matmul(self.P, sol[:, 2*self.nvar:3*self.nvar].transpose(0, 1))
#         x_pred = x_pred.transpose(0, 1)
#         y_pred = y_pred.transpose(0, 1)
#         psi_pred = psi_pred.transpose(0, 1)
#         out = torch.cat([x_pred, y_pred, psi_pred], dim=1)
#         return out
class TrajNet(nn.Module):
    def __init__(self, opt_layer, P, Pdot, input_size=32, hidden_size=64, output_size=3, nvar=11, t_obs=8):
    ...
    def forward(self, x, fixed_params, var_inp):
        ...
        # Run optimization
#         plt.scatter(fixed_params[1][0],fixed_params[1][1], color='black', label='inside')
#         plt.scatter(variable_params[1][0].detach(),variable_params[1][1].detach(), color='black', label='inside2')
#         print(fixed_params[1])
#         print(variable_params[1])
        sol = self.opt_layer(fixed_params, variable_params)
