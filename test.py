...
# plt.plot(x_lane_center, y_lane_center )
# plt.plot(x_mid*np.ones(prob.nvar), y_mid*np.ones(prob.nvar), 'og', markersize = 20.0)
# for i in range(0, np.shape(x_lane_aug)[0]):
	# x_circ = x_lane_aug[i]+x_init+prob.a_obs_lane*np.cos(th)
	# y_circ = y_lane_aug[i]+y_init+prob.b_obs_lane*np.sin(th)
