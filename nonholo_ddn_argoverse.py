...
class TrajNet(nn.Module):
    def __init__(self, opt_layer, P, Pdot, input_size=2, hidden_size=16, embedding_size = 2, output_size=2, nvar=11, t_obs=8, num_layers = 1):
        ...
        self.linear2 = nn.Linear(hidden_size, output_size + 1)
...
#         self.decoderlstm = nn.LSTM(hidden_size, output_size, num_layers, batch_first=True)
    ...
    def forward(self, x, fixed_params, var_inp):
        ...
        x = torch.tensor(x)
        out = x#self.activation(self.linear1(x))
        ...
        out = self.linear2(out)         # from (20, 20, 16) to (20, 20, 3)
        # out of shape (20, 20, 3)
        variable_params = out[:, -1, :].squeeze() # to shape (20, 3)
#         print(variable_params.shape)
...
#         variable_params = torch.cat((out[:,-1,:], torch.zeros(out.shape[0], 1)), dim=1)
