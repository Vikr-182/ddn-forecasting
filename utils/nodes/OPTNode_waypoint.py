...
class OPTNode_waypoint(AbstractDeclarativeNode):
    ...
    def solve(self, fixed_params, variable_params):
        ...
        x_fin, y_fin, psi_fin, x_mid1, y_mid1, x_mid2, y_mid2 = torch.chunk(variable_params, 7, dim=1)
        
        b_eq_x = torch.cat((x_init, x_mid1, x_mid2, x_fin), dim=1)
        b_eq_y = torch.cat((y_init, y_mid1, y_mid2, y_fin), dim=1)
    ...
    def objective(self, fixed_params, variable_params, y):
        ...
        cost_pos += 0.5*self.rho_eq*(torch.sum((x[:, 9] - x_mid1)**2, 1) + torch.sum((y[:, 9] - y_mid1)**2, 1) + torch.sum((x[:, 19] - x_mid2)**2, 1) + torch.sum((y[:, 9] - y_mid2)**2, 1))
